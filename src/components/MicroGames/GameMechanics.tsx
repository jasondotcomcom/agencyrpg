...
export interface CalSlot { day: number; slot: number; duration: number; title: string }
export interface CalMeeting {
  id: string; title: string; duration: number; color: string;
  constraint?: { days?: number[]; maxSlot?: number };
  trashable?: boolean; isSurprise?: boolean;
}
export interface CalScenario { name: string; prefilled: CalSlot[]; queue: CalMeeting[]; surpriseDelay: number }

const DAY_LABELS = ['Mon', 'Wed', 'Fri'];
const TIME_LABELS = ['9', '10', '11', '12', '1'];
const SLOTS = 8; // 8 half-hour slots = 4 hours (9am-1pm)
const SLOT_H = 20;
const DAYS = 3;

export function CalendarShuffleGame({
  scenario, onWin, onFail,
}: {
  scenario: CalScenario;
  onWin: (meta?: GameResultMeta) => void;
  onFail: (meta?: GameResultMeta) => void;
}) {
  void onFail; // parent timer handles timeout

  // Grid: grid[day][slot] = meeting id or null
  const [grid, setGrid] = useState<(string | null)[][]>(() => {
    const g: (string | null)[][] = Array.from({ length: DAYS }, () => Array(SLOTS).fill(null));
    for (const pf of scenario.prefilled) {
      const slots = pf.duration; // duration in half-hour slots
  ...
  // Total meetings that need action (place or trash)
  const totalRequired = scenario.queue.length;

  // Surprise meeting timer
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (placedMeetings.size + trashedSet.size >= totalRequired) {
      ...
      // Check if Focus Time was placed
      ...
      // Check if email meeting was trashed
      if ([...trashedSet].some(id => {
        const m = scenario.queue.find(q => q.id === id);
        return m?.trashable;
      })) flags.push('trashed-email');
      onWin({ customFlags: flags });
    }
  }, [placedMeetings.size, trashedSet.size, totalRequired, onWin, scenario.queue]);

  // Check if meeting fits at day/slot
  const canPlace = useCallback((meeting: CalMeeting, day: number, slot: number, currentGrid: (string | null)[][]) => {
    const slots = meeting.duration;
    if (slot + slots > SLOTS) return false;
    // Check overlap
    for (let s = 0; s < slots; s++) {
  ...
  useEffect(() => {
    ...
    const handleUp = (e: MouseEvent) => {
      ...
      if (gridEl) {
        ...
        // Check if over trash zone (below the queue panel)
        const trashEl = document.getElementById('cal-trash');
        if (trashEl) {
        ...
        // Calculate target day/slot from mouse position
        const timeLabelsWidth = 24;
  ...
  // Get constraint label for a meeting
  const getConstraintLabel = (m: CalMeeting) => {
    const parts: string[] = [];
    if (m.constraint?.days) {
      parts.push(m.constraint.days.map(d => DAY_LABELS[d]).join('/') + ' only');
    }
    if (m.constraint?.maxSlot !== undefined) {
      parts.push('before noon');
    }
    if (m.trashable) parts.push('trashable');
    return parts.join(' | ');
  };

  // Render duration as visual height
  const durationLabel = (d: number) => d === 1 ? '30m' : d === 2 ? '1hr' : d === 3 ? '1.5hr' : `${d * 30}m`;

  return (
    <div className={styles.calWrap}>
      {/* Calendar Grid */}
      <div ref={gridRef} className={styles.calGrid}>
        {/* Day headers */}
        ...
        {Array.from({ length: SLOTS }).map((_, s) => (
          <React.Fragment key={s}>
            {/* Time label (every 2 slots = 1 hour) */}
            <div className={styles.calTimeLabel}>
              {s % 2 === 0 ? TIME_LABELS[s / 2] : ''}
            </div>
            {/* Day cells */}
            {Array.from({ length: DAYS }).map((_, d) => {
              ...
              // For placed/prefilled blocks, only render on the first slot
              let blockEl: React.ReactNode = null;
              if (isPrefilled) {
                const pf = scenario.prefilled.find(p => `pf-${p.title}` === cellId && p.day === d && p.slot === s);
                if (pf) {
                  blockEl = (
                    <div className={styles.calPrefilled} style={{ height: pf.duration * SLOT_H - 2 }}>
                      {pf.title}
                    </div>
                  );
                }
                ...
                if (pos && pos.day === d && pos.slot === s) {
                  ...
                  if (meeting) {
                    blockEl = (
                      <div className={styles.calPlaced} style={{ height: meeting.duration * SLOT_H - 2, background: meeting.color }}>
                        {meeting.title}
                      </div>
                    );
                  }
              ...
              return (
                <div key={`${d}-${s}`}
                  className={`${styles.calSlot} ${isInvalid ? styles.calInvalid : ''}`}
                >
                  {blockEl}
                </div>
              );
      ...
      {/* Queue Panel */}
      <div className={styles.calQueue}>
        <div className={styles.calQueueTitle}>Meetings</div>
        {queueMeetings.map(m => (
          <div key={m.id}
            className={`${styles.calQueueItem} ${m.isSurprise && !placedMeetings.has(m.id) ? styles.calSurpriseIn : ''}`}
            style={{ borderLeftColor: m.color }}
            onMouseDown={(e) => {
              dragOffsetRef.current = { x: e.clientX - e.currentTarget.getBoundingClientRect().left, y: e.clientY - e.currentTarget.getBoundingClientRect().top };
              setDragId(m.id);
              setGhostPos({ x: e.clientX - dragOffsetRef.current.x, y: e.clientY - dragOffsetRef.current.y });
              e.preventDefault();
            }}
          >
            <div className={styles.calQueueItemTitle}>{m.title}</div>
            <div className={styles.calQueueItemDur}>{durationLabel(m.duration)}</div>
            {getConstraintLabel(m) && (
              <div className={styles.calConstraint}>{getConstraintLabel(m)}</div>
            )}
          </div>
        ))}

        {/* Trash zone */}
        <div id="cal-trash" className={styles.calTrash}>
          <span>🗑️</span> Trash
        </div>
      </div>
